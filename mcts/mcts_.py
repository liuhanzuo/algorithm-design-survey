...
def main(selection1 = None, selection2 = None):
    ...
    draw_rate = []
    for j in range(1, 20):
        args=([(number_of_games, number_of_games, False, 10 * j, 120, selection1, selection2)])
    ...
    plt.legend()
    plt.savefig(f'figs/120/{selection1} vs {selection2}1.png')
    win_rate1 = []
    win_rate2 = []
    draw_rate = []
    for j in range(1, 20):
        args=([(number_of_games, number_of_games, False, 120, 10*j, selection2, selection1)])
        win1, win2, draw = gaming.PlayGame(args[0])
        print("Player1 wins: %2.2f%%" % (win1/number_of_games*100))
        print("Player2 wins: %2.2f%%" % (win2/number_of_games*100))
        print("Draws: %2.2f%%" % (draw/number_of_games*100))
    ...
    plt.legend()
    plt.savefig(f'figs/120/{selection2} vs {selection1}1.png')
    win_rate1 = []
    win_rate2 = []
    draw_rate = []
    for j in range(1, 20):
    ...
    plt.title(f'{selection1} vs {selection2}')
    plt.legend()
    plt.savefig(f'figs/120/{selection1} vs {selection2}2.png')
